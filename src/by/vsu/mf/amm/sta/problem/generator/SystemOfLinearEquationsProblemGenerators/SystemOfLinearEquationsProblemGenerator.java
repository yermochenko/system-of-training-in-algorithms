...
import by.vsu.mf.amm.sta.math.object.TridiagonalMatrix;
...
public class SystemOfLinearEquationsProblemGenerator
        implements ProblemGenerator<SystemOfLinearEquationsProblem, TridiagonalMatrixAlgorithm> {
    ...
    @Override
    public void generate(SystemOfLinearEquationsProblem problem, TridiagonalMatrixAlgorithm method) {
        if (minSize > 0) {
            if (minSize <= maxSize) {
    ...
    private Matrix generateMatrix(Random random, int quantity) {
        TridiagonalMatrix matrix = new TridiagonalMatrix(quantity);
        double delta = (maxValue - minValue) / possibleValuesQuantity;
        //generate main diagonal
        for (int i = 0; i < quantity; i++) {
            matrix.set(i, i, minValue + delta * random.nextInt(possibleValuesQuantity + 1));
        }

        for (int i = 0; i < quantity - 1; i++) {
            matrix.set(i, i + 1, minValue + delta * random.nextInt(possibleValuesQuantity + 1));
    ...
    private void toDiagonallyDominantMatrix(Random random, Matrix matrix) {
        while (!isDiagonallyDominantMatrix(matrix)) {
            for (int i = 0; i < matrix.rowsQuantity(); i++) {
                ((TridiagonalMatrix) matrix).set(i, i, minValue + random.nextInt(possibleValuesQuantity + 1));
            }
        }
